...
- `src/tools/toolsets.ts` — capability keys (`tickets`/`time`/`companies`/`configurations`/`schedule`/`finance`/`advanced`/`sql`), persona presets (`tech`/`dispatch`/`invoicing`/`all`), and `resolveToolsets()`. Selection: `CW_TOOLSETS`/`--toolsets` (stdio, unknown key → `ConfigError`) or the `x-cw-toolsets` header (HTTP, unknown token ignored). `all` and `DEFAULT_TOOLSETS` are derived as `TOOLSET_KEYS` minus `OPT_IN_TOOLSET_KEYS` — so `advanced` is in `all` (never in a persona preset) while **`sql` is in neither** and must be named explicitly. `withoutDbToolsets()` is the no-database prune used by `createServer` and `sessionToolsets`
...
- `CW_DB_*` is environment-only (no CLI flags — `--db-password` would show in `ps`). Asking for `sql` without a database is a `ConfigError`; an HTTP session asking for it on a database-less server gets a warning and the key dropped
